...
public class LoginMain {
	...
	public static void main(String[] args) {
		...
		if (choix == 1) {
			...
			// piocher dans les utilisateurs du fichier
			// piocher dans les mots de passe du fichier
			if (UTILISATEUR_VALIDE.equals(utilisateurSaisi)) {
				if (MOTDEPASSE_VALIDE.equals(motDePasseSaisi)) {
					...
					do {
						...
						switch(saisie) {
						case 1 : System.out.println("Vous êtes dans l'écran de création");
								 break;
						case 2 : System.out.println("Vous êtes dans l'écran de mise à jour");
								 break;
						case 3 : System.out.println("Vous êtes dans l'écran de suppression");
								 break;
...
//			Files.write(Paths.get("motsdepasse.db"), nouveauMotDePasse.getBytes(), StandardOpenOption.APPEND, StandardOpenOption.CREATE);
			...
			try {
				Files.write(Paths.get("utilisateurs.db"), combinaison.getBytes(), StandardOpenOption.APPEND, StandardOpenOption.CREATE);
			} catch (IOException e) {
	...
	private static boolean login() throws LoginException{ // throws : indicateur qu'un type d'exception est susceptible d'etre généré par la méthode
		System.out.println("Entrez votre nom d'utilisateur : ");
		Scanner scanner = new Scanner(System.in);
		String utilisateurSaisi = scanner.nextLine();
		...
		String motDePasseSaisi = scanner.nextLine();
		if (UTILISATEUR_VALIDE.equals(utilisateurSaisi) && 
				MOTDEPASSE_VALIDE.equals(motDePasseSaisi)) {
			return true;
		} else { // on lance une LoginException avec un message explicatif
			throw new LoginException("Utilisateur et/ou mot de passe invalides"); // throw : je génère manuellement une exception
		}
	}
